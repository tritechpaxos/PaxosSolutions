...
//	LOG( pSession->s_pLog, LogDBG, "j=%d errno=%d", j, errno );
	...
	for( i = 0; i < pReq->m_N; i++ ) {
		...
		if( pVec->v_Len > 0 ) {
			...
			LOG( pSession->s_pLog, LogDBG,
				"%d:v_len=%d Ret=%d", i, pVec->v_Len, Ret );
...
int		
PaxosSessionDumpMem( struct Session *pSession, void *pHandle,
							void *pRemote, void *pLocal, size_t Length )
{
	PaxosSessionDumpReq_t	Req;
//	PaxosSessionDumpRpl_t	Rpl;
	int	Ret;

	SESSION_MSGINIT( &Req, PAXOS_SESSION_DUMP, 0, 0, sizeof(Req) );
	Req.d_Head.h_Id	= pSession->s_Id;
	Req.d_pStart	= pRemote;
	Req.d_Size		= Length;

	(pSession->s_fGetMyAddr)( pHandle, &Req.d_Head.h_Id.i_Addr );

	Req.d_Head.h_Cksum64	= in_cksum64( &Req, sizeof(Req), 0 );

	Ret = (pSession->s_fSendTo)( pHandle, (char*)&Req, sizeof(Req), 0 );
	if( Ret < 0 )	goto err;

	Ret = (pSession->s_fRecvFrom)( pHandle, (char*)pLocal, Length, 0 );
	if( Ret < 0 )	goto err;
