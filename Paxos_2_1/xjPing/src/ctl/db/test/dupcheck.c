...
int
insert_table( r_man_t *pMan )
{
	r_tbl_t	*pTbl;
	char	*pBuf;
	int		Ret;

	if( !(pTbl = rdb_open( pMan, TABLE ) ) ) {
			goto err1;
	}
	pBuf	= AllocRec( pTbl );

	Ret = rdb_insert( pTbl, REC_NUM, (r_head_t*)pBuf, 2, check, R_EXCLUSIVE );
	...
	char	*av[];
{
	...
	if( ac < 4 ) {
		printf("usage:test db_man db_local {create|inser}\n");
		return( 1 );
	}
