...
if [ $# -lt 2 ]
then
echo "USAGE:css_monitor.sh ras_cell service"
exit 1
fi
...
do
	...
	echo "ssh -f $host \"cd $root;$env $bin/PFSServer $service $id $root TRUE 0 3>&1 1>&2 2>&3 | $bin/LogFiles CSS $files\""

	ssh -f $host "cd $root;$env $bin/PFSServer $service $id $root TRUE 0 3>&1 1>&2 2>&3 | $bin/LogFiles CSS $files"
	...
	then
		...
		then
			...
			then
				...
				ssh -f $host "cd $root;$env $bin/PFSServer $service $id $root FALSE 3>&1 1>&2 2>&3 | $bin/LogFiles CSS $files"
