...
#ifdef	ZZZ
		Ret = QueueWaitEntry( &WorkQueue, pWM, wm_Lnk );
...
_VVReadRequest(Vol_t *pVl, VV_t *pVV, char *pBuff, 
						size_t Length, off_t Offset, VV_IO_t *pVVIO )
#else
int
_VVReadRequest(Vol_t *pVl, VV_t *pVV, char *pBuff, 
...
VVReadRequest(Vol_t *pVl, VV_t *pVV, char *pBuff, size_t Length, off_t Offset)
{
	...
	Ret = _VVReadRequest(pVl, pVV, pBuff, Length, Offset, &VVIO );
	if( Ret < 0 )	goto err;
		...
		if( !list_empty( &BC_IO.io_Lnk ) ) {
			...
			list_for_each_entry( pReq, &BC_IO.io_Lnk, rq_Lnk ) {
				...
				Ret = _VVReadRequest(pVl, pVV, pReq->rq_pData, 
						pReq->rq_Size, pReq->rq_Offset, &VVIO );
			}
			Ret = VVKickReqIO( &VVIO, WM_READ ); // Kick and Wait
...
{
	if( pVSDL->l_pDlg ) {
		if( pVSDL->l_pDlg->vd_Dlg.d_Dlg.d_Own == PFS_DLG_W ) {
			DlgCacheReturn( &pVSDL->l_pDlg->vd_Dlg, NULL );
		}
		LOG( pLog, LogINF, 
		"VUnlock:Dlg[%s] uDlg=%"PRIu64"", 
		pVSDL->l_pDlg->vd_Dlg.d_Dlg.d_Name, pVSDL->l_uDlg );

		VUnlock( pVSDL->l_pDlg );
	}
	return( 0 );
}
...
VSDLockW( VSDlgLock_t *pVSDL, VV_t *pVV, uint64_t uVS )
{
	...
	if( pVSDL->l_pDlg ) {
		if( pVSDL->l_uDlg != uDlg ) {
			...
			LOG( pLog, LogINF, 
			"VUnlock-VLockW:VV[%s] Dlg[%"PRIu64"] VS[%"PRIu64"]", 
					pVV->vv_DB.db_Name, uDlg, uVS );

			pVSDL->l_pDlg = VLockW( pVV, uVS );
			pVSDL->l_uDlg	= uDlg;
		} else {
			if( pVSDL->l_pDlg->vd_Dlg.d_Dlg.d_Own == PFS_DLG_R ) {
...
VSDLockR( VSDlgLock_t *pVSDL, VV_t *pVV, uint64_t uVS )
{
	...
	if( pVSDL->l_pDlg ) {
		if( pVSDL->l_uDlg != uDlg ) {
			...
			LOG( pLog, LogINF,
			"VUnlock-VLockR:VV[%s] Dlg[%"PRIu64"] VS[%"PRIu64"]", 
			pVV->vv_DB.db_Name, uDlg, uVS );

			pVSDL->l_pDlg = VLockR( pVV, uVS );
			pVSDL->l_uDlg = uDlg;
		}
	} else {
...
int
_VVWriteRequest( Vol_t *pVl, VV_t *pVV, 
			char *pBuff, size_t Length, off_t Offset, VV_IO_t *pVVIO )
{
	uint64_t	iUnit;
	uint64_t	Off, End, Len, end;
	int	Ret;
				...
				char *pBuff, size_t Length, off_t Offset )
{
	...
	Ret	= _VVWriteRequest( pVl, pVV, pBuff, Length, Offset, &VVIO );
	if( Ret < 0 )	goto err;
