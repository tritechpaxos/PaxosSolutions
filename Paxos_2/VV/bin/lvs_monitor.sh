...
if [ $# -lt 2 ]
then
echo "USAGE:lvs_monitor.sh ras_cell service"
exit 1
fi
...
do
	...
	echo "ssh -f $host \"cd $root;$env $bin/LVServer $service $id $root TRUE 0 3>&1 1>&2 2>&3 | $bin/LogFiles $service $files\""

	ssh -f $host "cd $root;$env $bin/LVServer $service $id $root TRUE 0 3>&1 1>&2 2>&3 | $bin/LogFiles $service $files"
