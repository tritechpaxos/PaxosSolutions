...
#ifdef	ZZZ
int
LVSessionShutdown( PaxosAcceptor_t *pAcceptor, PaxosSessionHead_t *pM )
{
...
#ifdef KKK
		Ret = PFSRasThreadCreate( pSession, pReqRAS->r_Path, 
...
#ifdef	ZZZ
	pPaxosCell->c_Max		= Core;
	pPaxosCell->c_Maximum	= Maximum;
	pSessionCell->c_Max		= Core;
	pSessionCell->c_Maximum	= Maximum;
#endif
/*
 *	Log
 */
...
#ifdef	ZZZ
	if( bAutonomic ) {
		if( target_id >= 0 ) 	autonomic( pAcceptor, target_id );
		else 					autonomic( pAcceptor, -1 );
	}
#endif
 ...
 */
#ifdef	ZZZ
	if( PaxosAcceptorStart( pAcceptor ) < 0 ) {
		printf("START ERROR\n");
		exit( -1 );
	}
#else
