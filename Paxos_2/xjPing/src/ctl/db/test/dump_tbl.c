...
DBG_ENT(M_RDB,"neo_main");
	...
	if( ac < 4 ) {
		printf("DUMP tbl structure\n");
		printf("usage:dump_tbl db_man db_local TABLE\n");
		neo_exit( 1 );
	}
DBG_A(("db_man=[%s],my_port=[%s],table=[%s]\n", av[1], av[2], av[3] ));
	...
	if( rdb_dump_mem( md, tbl.t_tag, sizeof(my_tbl_t), &mytbl ) < 0 )
		goto err1;
	...
	r_tbl_t	*td;
{
	...
	printf("type		= %d\n",	td->t_type );
	printf("midified time	= %s",	ctime(&td->t_modified) );
	printf("stat		= 0x%x\n",	td->t_stat );
	printf("user count	= %d\n", 	td->t_usr_cnt );
