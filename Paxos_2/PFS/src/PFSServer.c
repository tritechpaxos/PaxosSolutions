...
#ifdef	ZZZ
		Ret = pfsUnlock( pAccept, &pAccept->a_Id, pJoint );
#else
...
#ifdef	ZZZ
			pfsUnlock( pAccept, &pAccept->a_Id, pJoint );
#else
...
pfsUnlock( struct PaxosAccept *pAccept, 
			PaxosClientId_t* pId, PFSJoint_t* pJoint )
#else
int
pfsUnlock( struct PaxosAccept *pAccept, 
			PaxosClientId_t* pId, PFSJoint_t* pJoint, bool_t bAbort )
...
ReadDir( DIR *pDir )
{
	...
	while( (pDirEnt = readdir( pDir )) ) {
		...
		/* skip XXX .CKSUM */
		if( !strstr( pDirEnt->d_name, BLOCK_CKSUM_FILE_SUFFIX ) )	break;
	}
	return( pDirEnt );
}
...
PFSReadDirS( struct PaxosAccept* pAccept, PFSHead_t* pH )
{
	...
	pMsgRpl	= MsgCreate( pReq->r_Number + 1,
								PaxosAcceptGetfMalloc(pAccept),
	...
	MY_REAL_PATH( pReq->r_Name );

	char dirnam[PATH_NAME_MAX], basenam[FILE_NAME_MAX];
	strncpy( dirnam, PathName, sizeof(dirnam) );
	strncpy( basenam, PathName, sizeof(basenam) );
	...
	for( i = 0; i < pReq->r_Index; i++ ) {
		if( !ReadDir( pDir ) ) {
			goto ret;
		}
	}
	n	= 0;
	while( (n < pReq->r_Number ) && (pDirent = ReadDir( pDir )) ) {
		pFileDirent	= 
			(PFSDirent_t*)MsgMalloc( pMsgRpl, sizeof(PFSDirent_t));	
		strncpy( pFileDirent->de_Name, pDirent->d_name, 
							sizeof(pFileDirent->de_Name) );
		MsgVecSet( &Vec, VEC_MINE, pFileDirent, 
				sizeof(*pFileDirent), sizeof(*pFileDirent) );
		MsgAdd( pMsgRpl, &Vec );


		pRpl->r_Number++;
		pRpl->r_Head.h_Head.h_Len	+= sizeof(*pFileDirent);
		n++;
	}
	closedir( pDir );
...
#ifdef	ZZZ
		strncpy( pFileDirent[i].de_Name, pDirent[i].d_name, 
							sizeof(pFileDirent[i].de_Name) );
#else
